![icon](GarbageCleaner/GarbageCleaner/favicon.ico) 
# Windows 垃圾文件清理工具
[![Passing](https://img.shields.io/badge/Cleaner-Passing-green/?logo=github)](https://github.com/CLoneLING/WindowsGarbageCleaner)
[![Support](https://img.shields.io/badge/Windows-支持-green/?logo=data:image/svg+xml;charset=utf-8;base64,)](https://github.com/CLoneLING/WindowsGarbageCleaner)
[![Buildon](https://img.shields.io/badge/Build_on-C%23-green/?logo=dotnet)](https://learn.microsoft.com/zh-cn/dotnet/csharp/)

这是一款可以清理Windows系统中垃圾文件的工具，如Temp临时文件、Prefetch缓存等.

支持语言：简体中文, [English](README.EN.md)
<br><br>
    ![UI](UI_main.png)
## 更新了什么？ 

这些是所有已发布版本更新的内容说明. →

- ## 正式版
  已发布正式版本的更新说明.
 - #### ~~Version 1.0.0~~
  1. ~~基本设计脚本.~~<br>已删除.
 - #### Version 1.1.1
  1. 基于 .NET Framework 4.8，所有对象已由 Batch 批处理脚本重构为 C# 程序.
  2. 全新的UI设计.
  3. 图标更新.
 - #### Version 1.1.2
  1. 新增：项目主页、问题反馈 选项.
  2. 新增：检查更新 功能及优化.
  3. 接入GitHub API，支持程序自动检查更新，以确保程序版本领先（需连接网络）.
 - #### Version 1.1.3
  1. 新增：实时时间显示.
  2. 新增：天气及温度实时显示系统 功能与优化.
  3. API调用实现天气温度实时实地显示（需连接网络）.
 - #### Version 1.2.0
  1. 新增：可开启 开机自动清理 功能.
  2. 新增：对更新系统增添友好性便利性优化更新及UI优化，可在应用内直接预览更新内容、进行一站式下载和安装.
  3. 修复了：在部分高DPI缩放比例的显示器上容易出现显示模糊、错位等异常的问题.
  4. 优化了：清理功能，更好地协调性能.


- ## 测试版
  已发布测试版的更新说明.
 - #### Version 1.1.0.beta3
  1. 添加问候语，引用您的用户名并显示.
 - #### Version 1.1.0.beta5
  1. 优化UI显示，以适配在不同显示比例下的显示方式.
  2. 解决了进度条因部分文件无法处理而跑不满的问题.
  3. 全局应用新图标.
